...
public class P4949 {
	 ...
	 * 견이 하나 이상 있다면, 문제의 난이도는 모든 사람의 난이도 의견의 30% 절사평균으로 결정한다.
	 * 사평균이란 극단적인 값들이 평균을 왜곡하는 것을 막기 위해 가장 큰 값들과 가장 작은 값들을 제외하고 평균을 내는 것을 말한다. 30% 절사평균의 경우 위에서 15%, 아래에서 15%를 각각 제외하고 평균을 계산한다. 따라서 20명이 투표했다면, 가장 높은 난이도에 투표한 3명과 가장 낮은 난이도에 투표한 3명의 투표는 평균 계산에 반영하지 않는다는 것이다.
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0;i < n;i++) {
			sb.append(confirm(br.readLine())).append("\n");
		}
	...
	//검증 메소드 생성
	public static String confirm(String word) {
		...
		for(int i = 0;i < word.length();i++) {
			...
			if(c == '(' || c == '[') {
				...
				if(s.empty() || s.peek() != '(') { //들어오는거에 맞춰서 확인
					return "NO";
				}else {
					s.pop();
				}
